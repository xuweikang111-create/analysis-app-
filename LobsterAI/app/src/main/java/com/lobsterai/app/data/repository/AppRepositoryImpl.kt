...
class AppRepositoryImpl @Inject constructor(
    ...
    override fun observeLobsters(): Flow<List<Lobster>> = db.lobsterDao().observeAll().map { list -> list.map { it.toDomain() } }
    override fun observeLobster(id: Long): Flow<Lobster?> = db.lobsterDao().observeById(id).map { it?.toDomain() }

    override suspend fun createLobster(name: String, prompt: String): Long = db.lobsterDao().insert(
        LobsterEntity(name = name.trim().ifBlank { "小龙" }, prompt = prompt.trim())
    )

    override suspend fun updateLobster(lobster: Lobster) = db.lobsterDao().update(lobster.toEntity())
    override suspend fun deleteLobster(lobster: Lobster) = db.lobsterDao().delete(lobster.toEntity())

    override suspend fun applyGrowth(lobsterId: Long, exp: Int, mood: Int, satiety: Int, intimacy: Int) {
        val current = db.lobsterDao().getById(lobsterId) ?: return
        val nextExp = max(0, current.experience + exp)
        val nextLevel = levelForExp(nextExp)
    ...
    override fun observeConversations(lobsterId: Long): Flow<List<Conversation>> =
        db.conversationDao().observeByLobster(lobsterId).map { list -> list.map { it.toDomain() } }

    override suspend fun createConversation(lobsterId: Long, title: String, modelConfigId: Long?): Long =
        db.conversationDao().insert(ConversationEntity(lobsterId = lobsterId, title = title.ifBlank { "新对话" }, modelConfigId = modelConfigId))

    override suspend fun deleteConversation(conversation: Conversation) = db.conversationDao().delete(conversation.toEntity())
    override fun observeMessages(conversationId: Long): Flow<List<Message>> = db.messageDao().observeByConversation(conversationId).map { list -> list.map { it.toDomain() } }
    override suspend fun getMessages(conversationId: Long): List<Message> = db.messageDao().getByConversation(conversationId).map { it.toDomain() }
    ...
    override suspend fun deleteMessage(message: Message) = db.messageDao().delete(message.toEntity())
    override suspend fun deleteMessagesFrom(conversationId: Long, messageId: Long) = db.messageDao().deleteFrom(conversationId, messageId)

    override fun observeModelConfigs(): Flow<List<ModelConfig>> = db.modelConfigDao().observeAll().map { list -> list.map { it.toDomain() } }
    override suspend fun getModelConfig(id: Long): ModelConfig? = db.modelConfigDao().getById(id)?.toDomain()
    ...
    override suspend fun saveModelConfig(
        ...
        return if (existing == null) db.modelConfigDao().insert(entity) else {
    ...
    override suspend fun testModelConnection(config: ModelConfig, apiKeyOverride: String?): Result<String> = withContext(Dispatchers.IO) {
    ...
    override fun observeDailyProgress(lobsterId: Long): Flow<DailyProgress> {
        ...
        return db.dailyActionDao().observeDay(lobsterId, day).map { list ->
            ...
            DailyProgress("checkin" in actions, "feed" in actions, "interact" in actions, "chat" in actions)
        }
    }

    override suspend fun markDailyAction(lobsterId: Long, action: String): Boolean {
        val inserted = db.dailyActionDao().insert(DailyActionEntity(lobsterId = lobsterId, dayKey = LocalDate.now().toString(), action = action))
        return inserted > 0
    }
